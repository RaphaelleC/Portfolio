function Interests() {
  return (
    <main className="text-center" id="interests">
      <br/>
      <br/>
      <br/>
      <div className="container">
        <div className="row">
          <h1 className="fw-bold text-danger mb-5">My Interests</h1>
          <hr className="border"></hr>
        </div>
      </div>
      <div className="container">
        <div className="row">
          <div className="m-5 col">
            <p className="fs-5">I&apos;ve had the chance to travel in a few countries, like Japan, Spain (Barcelona), Italy (Rimini) and in the French Alps (I thrive when snowboarding). I am looking forward to my next adventures ! Discovering new ways of living, thinking and approaching life is such an enriching experience.</p>
          </div>
        </div>
      </div>
      <div className="row">
        <div className="col">
          <img src="https://i.imgur.com/6NveE7t.jpg" alt="French Alps" className="picture"/>
        </div>
        <div className="col">
          <img src="https://i.imgur.com/1a9wO2v.jpg" alt="Sensoji Temple" className="picture"/>
        </div>
      </div>
    </main>
  )
}

export default Interests